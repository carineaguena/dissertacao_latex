...
Esta pesquisa propõe o ETL4NoSQL que é um framework para desenvolvimento de aplicações ETL para sistemas que utilizam bases de dados NoSQL, com poucas definições de esquemas e não relacionais. 
Bases de dados NoSQL são utilizadas em muitas aplicações modernas, principalmente aplicações Web, onde possuem um grande volume e variedade de dados. 
...
As bases de dados relacionais utilizam uma linguagem de manipulação padrão conhecida por SQL (Structure Query Language), porém as bases de dados NoSQL não possuem uma linguagem em comum, como as relacionais, onde cada SGBD possui sua própria linguagem de manipulação de dados. Por isso, é essencial que haja um mecanismo que integre a leitura e escrita das bases NoSQL. Dessa forma, o ETL4NoSQL deve possuir um ambiente que importe os dados das diversas bases NoSQL, e que possa fazer a leitura e escrita dos dados. 

Muitos sistemas de BI utilizam o modelo relacional como referência


Requisitos funcionais:

O sistema deve importar os dados de diversas bases relacionais e não relacionais.

O sistema deve permitir a leitura e escrita dos dados.

O sistema deve permitir organizar os dados no modelo relacional.

O sistema deve permitir organizar os dados em quaisquer modelo desejado pelo usuário.

O sistema deve possuir os mecanismos ETL mais conhecidos na literatura.

O sistema deve possibilitar a criação de novos mecanismos ETL desejado pelo usuário.

O sistema deve possuir um ambiente que possibilite a execução dos mecanismos de ETL em operações.




Dessa forma, para auxiliar o desenvolvimento de processos de ETL utilizando um sistema que possui vários tipos de bases de dados, os requisitos funcionais elencados do ETL4NoSQL são:


Dessa forma, os requisitos funcionais e não funcionais do ETL4NoSQL estão divididos em quatro principais componentes de software. O uso de componentes de software para o desenvolvimento do framework proposto é baseada na possibilidade do reuso, reduzindo o trabalho repetitivo na implementação dos componentes e o custo de manutenção (Szyperski, et al.2002). 
...
Dessa forma, baseado nos requisitos elencados do ETL4NoSQL, optamos por definir a arquitetura baseada em componentes como a arquitetura a ser seguida no desenvolvimento do framework. Com a arquitetura em componentes será possível fazer o reuso dos componentes e também facilitará na manutenção e melhoria.
...
\item[d)] Participantes: Apresenta os elementos do ambiente e estrutura de dados.
	...
	\begin{figure}[h!]
		...
		\includegraphics[scale=0.5]{fig/Diagrama_classe_repositorio.png}
		\label{arquitetura}
	\end{figure}
...
\item[d)] Participantes: Apresenta os elementos dos mecanismos comuns e personalizados.
	...
	\begin{enumerate}
		...
		\begin{figure}[h!]
			...
			\includegraphics[scale=0.5]{fig/Metadados_mecanismo.png}
			\label{meanismo}
		\end{figure}
	...
	\item[c)] Estrutura: A operação de leitura é feita por meio da instanciação dos elementos:
		...
		\begin{enumerate}
			...
			\begin{figure}[h!]
				...
				\includegraphics[scale=0.5]{fig/fluxo_leitura_ambiente.png}
				\label{fluxograma}
			\end{figure}
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.5]{fig/diagrama_componentes.png}
...
\end{figure}
 ...
 \begin{itemize}
	...
	\item[e)] Customização: Este componente pode ser customizado em sua interface com o usuário, possibilitando outras meios de fornecer as informações para os seus métodos
	
	\item[f)] Suporte a evolução: Deve possibilitar o suporte aos métodos de acordo com as mudanças de conexões e manipulações de bases de dados futuras
 ...
 \end{itemize}
  ...
  \begin{itemize}
	\item[a)] Interface: Componente responsável pela criação de ambientes para a execução de processos de ETL, importação e mapeamento NoSQL.
...
\subsection{Definição de Inventário NoSQL}

A partir da análise de padrões e definição dos componentes de Importação e Mapeamento foi possível projetar o modelo de interação entre esses componentes, mostrado na figura \ref{interacao_nosql}. 
%figura modelo de interação inventário.
Ele fornece uma visão conceitual da relação entre os componentes para a construção do inventário aplicado na importação dos dados das bases NoSQL e no mapeamento para o esquema relacional utilizado na fase de ETL. 

\begin{figure}[h!]
\centering
\includegraphics[scale=0.5]{fig/diagrama_classe_interacao_nosql.png}
\caption{Diagrama de Classes Modelo de Interação entre Componentes}
\label{interacao_nosql}
\end{figure}

%criar uma figura com o modelo de interação entre o os componentes importação e mapeamento 

O inventário NoSQL é composto pelas entidades:

\begin{itemize}
	\item Inventário Metadados: é a entidade responsável por manter, adicionar e remover um conjunto de inventários;
	
	\item Esquema de dados: entidade que define o comportamento e os atributos de um esquema de dados, é possível derivar outras entidades de esquema específicos a partir dela;
	
	\item Inventário: entidade que mantêm a descrição do inventário, dados de conexão e o conjunto de esquemas que precisam estar disponíveis para a importação e mapeamento do dados NoSQL;
	
	\item Sintax DDL: entidade que define o comportamento e os atributos da linguagem de definição de dados de um SGBD;
	
	\item Sintax DML: entidade que define o comportamento e os atributos da linguagem de manipulação de dados de um SGBD.
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.5]{fig/diagrama_classe_inventario.png}
\caption{Diagrama de Classes do Componente Inventário}
\label{inventario}
\end{figure}

\subsubsection{Instanciação do Inventário NoSQL}
...
%Para o Desenvolvimento dos Componentes foram feitas conexões com os principais SGDBs dos paradigmas NoSQL discutidos nesta pesquisa. Dessa forma, torna-se essencial a instalação desses SGBDs para fim de testar as funcionalidades do Framework.
